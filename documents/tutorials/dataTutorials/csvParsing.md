...
toMarkdown { header, body } =
    let
        divider =
            toTableRow (List.repeat (List.length header) "-")
    ...
    toTableRow header
        :: divider
        :: List.map toTableRow body
```

And review the output when applied to the titanic CSV input:

^^^elm m=(parse csv titanic |> toMarkdown)^^^

Once parsed we can easily perform other operations on the input, such as showing the details of all who survived the Titanic disaster:
...
survivors =
    let
        { header, body } =
            parse csv titanicFull

        survived =
            body |> List.filter (List.drop 1 >> List.head >> (==) (Just "1"))
    in
    Table header survived |> toMarkdown
```
